...
for count in range(0,len(points_file[:,0])):
    ...
    mags, temperature, gravity, metallicity = InterpolateMagnitudes(data_file, temp_grid, metallicity_grid, gravity_grid)
    output = np.empty((len(temperature),9))
    output[:,0:6], output[:,6], output[:,7], output[:,8] = mags[:,0:6], temperature, gravity, metallicity
    np.savetxt('interp_'+str(temp)+"_"+str(z)+"_"+str(g)+'.txt', output, header = 'LSST_U_Mag LSST_G_Mag LSST_R_Mag LSST_I_Mag LSST_Z_Mag LSST_Y4_Mag Temperature Gravity Metallicity')
    stats = np.empty(15)
