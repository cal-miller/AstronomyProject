...
def RunSEDs(outFileName, dataPath='/Users/fluffy/Documents/workspace/AstronomyProject/data/', SEDPath='/home/fluffy/workspace/GaiaSims/phoenix', points_file = 'SamplePoints.txt'):
    ...
    for filename in listing:
        ...
        if m:
            ...
            for a in range(len(points)):
                if (points[a,0] < (temp + 500)) & (points[a,0] > (temp - 500)) &  (points[a,1] < (g + 1.5)) & (points[a,1] > (g - 1.5)) & (points[a,2] < (z + 1.5)) & (points[a,2] > (z - 1.5)):
                    run = True
        if (m or n) and run and not fnmatch.fnmatch(filename, "*Alpha*"):
