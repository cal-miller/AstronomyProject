...
dataDict = {"Gaia_G_mag":0, "LSST_U_Mag":1,  "LSST_G_Mag":2,  "LSST_R_Mag":3,  "LSST_I_Mag":4,  "LSST_Z_Mag":5,  "LSST_Y4_Mag":6, "T":7, "G":8, "Z":9, "a0":10}
pointsDict = {"T":0, "G":1, "Z":2, "a0":3}
data_file = np.loadtxt('Mags_g.txt')
points_file = np.loadtxt('SamplePoints.txt')
stats_file = open("InterpStats.txt" , 'w')
print >>stats_file, '# LSST_U_Mag_SD LSST_U_Mag_Skew LSST_G_Mag_SD LSST_G_Mag_Skew LSST_R_Mag_SD LSST_R_Mag_Skew LSST_I_Mag_SD LSST_I_Mag_Skew LSST_Z_Mag_SD LSST_Z_Mag_Skew LSST_Y4_Mag_SD LSST_Y4_Mag_Skew Temperature Gravity Metallicity a0'
for count in range(0,len(points_file[:,0])):
    ...
    a0_grid = np.arange(a0+.5,a0-.5,.05)#0 to 1
    ...
    np.savetxt('interp_'+str(temp)+"_"+str(z)+"_"+str(g)+"_"+str(a0)+'.txt', output, header = 'LSST_U_Mag LSST_G_Mag LSST_R_Mag LSST_I_Mag LSST_Z_Mag LSST_Y4_Mag Temperature Gravity Metallicity a0')
    stats = np.empty(16)
    for band in range(0,6):
        stats[2*band], stats[2*band+1] = np.std(output[:,band]), stat.skew(output[:,band])
    
    stats[12], stats[13], stats[14], stats[15] = temp, g, z, a0
    print >>stats_file, stats[0],stats[1],stats[2],stats[3],stats[4],stats[5],stats[6],stats[7],stats[8],stats[9],stats[10],stats[11],stats[12],stats[13],stats[14],stats[15]